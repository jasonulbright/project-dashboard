...
- **State banner** — surfaces merge / rebase / cherry-pick / revert / bisect / detached-HEAD / conflicts loudly, with an "Open in Terminal" escape hatch that is never taken away, and — for everything but bisect — a way into the conflict panel below
- **Conflicts** — a panel that drives the operation a repository is stopped in, not a merge editor: every unmerged file with the shape git recorded for it, its stages rendered read-only in the same diff view as everywhere else (base against ours, base against theirs, ours against theirs), and per file take ours, take theirs, open it in your editor with its markers, or stage what you resolved by hand. Continue or abort the merge, rebase, cherry-pick or revert from there, both confirmed, with the commit message shown and editable before a continue writes it, and the signing question asked first in a repository that signs. Staging a file that still holds conflict markers is refused outright and there is no override; a submodule conflict, a path neither side kept, and a rebase started outside the app each say why they cannot be handled here — and abort and Open in Terminal stay one click away from every state the panel can produce
